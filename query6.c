...
typedef struct Student {
    ...
    int negative;               // For filtering purpose
} Student;
...
int main(int argc, char *argv[]) {
	...
	if (db){							
		...
		for (int i = 0; i < size; ++i){ // process all the student records in database
            ...
            s.negative = 1;
			// Checking if student has a negative grades
			for(int j = 0; j < s.load; ++j)
			{
				if(s.grades[j] < 5)
				{
					s.negative = 0;		// If student has negative grades, s.negative will be set to 0
				}
			}


			if(s.negative != 0){ // *** first filter, conditions on the student
