...
int main(int argc, char *argv[]) {
	...
	if (db){							
		...
		for (int i = 0; i < size; ++i){ // process all the student records in database
            ...
            s.negative = 0;
			// Checking if student's grades is between 6 and 9         6 <= grades <= 9
			for(int j = 0; j < s.load; ++j)
			{
				if(s.grades[j] >= 6 && s.grades[j] <= 9)
				{
                    
					s.negative++;		// If student's grade is between 6 and 9, 1 will be added to s.negative
				}
			}
            bool good_grades = false;
            if(s.negative == s.load)    // If student's load is == to grades that is between 6 and 9, bool value will be set to true
            {
                good_grades = true;
            }

			if(good_grades){ // *** first filter, conditions on the student
				...
				for (int i = 0; i < s.load; ++i){ // process each course taken by the student
					if(1){ // *** second filter, conditions on the course/grade
						...
						printf("%s GRADE [%d] ", s.courses[i], s.grades[i]);
					}
				}
				printf("%s\n", s.languages);	
