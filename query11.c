...
int main(int argc, char *argv[]) {
	...
	if (db){							
		...
		for (int i = 0; i < size; ++i){ // process all the student records in database
            ...
            s.negative = 0;
			// Checking if student has Philosophy or have a 10
            ...
            bool philosophy_or_10 = false;
            if(s.negative != 0)    // If student has Philosophy or have a 10
            {
                philosophy_or_10 = true;
            }

			if(philosophy_or_10){ // *** first filter, conditions on the student
