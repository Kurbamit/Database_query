...
int main(int argc, char *argv[]) {
	...
	if (db){							
		...
		for (int i = 0; i < size; ++i){ // process all the student records in database
            ...
            s.negative = 0;
			// Checking if student spekas Lithuanian language
            char *a = strstr(s.languages, "Lithuanian");
            if(a != NULL)
            {
                s.negative++;
            }
            
            bool lithuanian_language = false;
            if(s.negative != 0)    // If student speaks Lithuanian language, then bool will be set to true
            {
                lithuanian_language = true;
            }

			if(lithuanian_language){ // *** first filter, conditions on the student
