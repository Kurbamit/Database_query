...
int main(int argc, char *argv[]) {
	...
	if (db){							
		...
		for (int i = 0; i < size; ++i){ // process all the student records in database
            ...
            s.negative = 0;
			// Checking if student has an Algebra and a Calculus
			for(int j = 0; j < s.load; ++j)
			{
                for(int k = 0; k < s.load; ++k)
                {
                    if(!strcmp(s.courses[j], "Algebra") && !strcmp(s.courses[k], "Calculus"))
                    {
                        s.negative++;
                    }
                }
			}
            bool calculus_and_algebra = false;
            if(s.negative != 0)    // If student has calculus and algebra, bool value will be set to true
            {
                calculus_and_algebra = true;
            }

			if(calculus_and_algebra){ // *** first filter, conditions on the student
